...
由于 Kibana 插件开发完之后接口都是固定的。由于插件更新很快，因此很难向后兼容。Kibana 强制要求安装的插件版本必须和 Kibana 版本一致。插件开发者必须为每个新的 Kibana 版本发布新的插件版本。
...
您可以通过简单的指定插件名称来下载 Elastic 官方插件。也可以指定插件具体的 URL 来下载安装，例如：
...
如果插件使用了不同的用户安装且服务又没有跑起来，您就需要修改这些文件的所属用户：
...
您也可以通过手动删除 `plugins/` 目录下的对应的插件目录来删除插件。
...
在 `install` 和 `remove` 命令中使用 `-c` 或者 `--config` 选项来指定启动 Kibana 启动的配置文件的路径。默认情况下，Kibana 使用 `config/kibana.yml` 配置文件。当您需要修改已安装好的插件配置时，使用 `bin/kibana-plugin` 命令来重启 Kibana 服务。当您使用自定义的配置文件时，每次使用 `bin/kibana-plugin` 命令必须指定配置文件的路径。
