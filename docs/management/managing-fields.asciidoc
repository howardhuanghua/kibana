...
当您在 Kibana 中定义一个脚本化字段时，您可以选择使用的脚本语言。从 5.0 版本开始，默认的选项是 {es-ref}modules-scripting-expression.html[Lucene  表达式] 和 {es-ref}modules-scripting-painless.html[Painless]。如果您在 Elasticsearch 中启用了其它动态脚本语言，也可以在 Kibana 中使用，但是不建议这样做。因为它们可能会不够安全 {es-ref}modules-scripting-security.html[sandboxed]。
...
WARNING: 请注意，脚本化字段没有内置验证。如果您的脚本有问题，每次当您尝试查看动态生成的数据时，您都会遇到异常。
