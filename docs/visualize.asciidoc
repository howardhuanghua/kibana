...
[partintro]
--
_可视化 (Visualize)_ 功能可以为您的 Elasticsearch 数据创建可视化控件。之后，您就可以创建<<dashboard, 仪表板>>将这些可视化控件整合到一起展示。

Kibana 可视化控件基于 Elasticsearch 的查询。利用一系列的 Elasticsearch {es-ref}search-aggregations.html[查询聚合]功能处理数据，您可以通过创建图表来呈现您关心的数据分布和趋势。
