...
[partintro]
--
_可视化 (Visualize)_ 功能可以为您的 Elasticsearch 数据创建可视化控件。之后，您就可以创建<<dashboard, 仪表盘>>将这些可视化控件整合到一起展示。

Kibana 可视化控件基于 Elasticsearch 的查询。利用一系列的 Elasticsearch {es-ref}search-aggregations.html[查询聚合]功能处理数据，然后利用一些图表来呈现您关心的数据趋势分布。
...
比如，如果正在索引 Apache 服务器日志，就可以构建一个 bar 图，通过指定 `geo.src` 字段上的一个 term 聚合，来展示地理位置的请求分布：
